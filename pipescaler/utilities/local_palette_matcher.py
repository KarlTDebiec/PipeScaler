...
class PaletteMatcher:
    ...
    def match_palette(
        ...
        if self.palette_match_mode == PaletteMatchMode.BASIC:
            ...
            start = time.time()
            best_fit_palette = self.get_basic_match_rgb(fit_palette, ref_palette)
    ...
    def get_basic_match_rgb(
        ...
        for i, color in enumerate(fit_palette):
            ...
            best_fit_palette[i] = cls.get_basic_match_for_color(
    ...
    def get_basic_match_for_color(
        ...
        for cell_to_check in cells_to_check:
            # noinspection PyTypeChecker
            best_dist_in_cell, best_color_in_cell = get_closest_color_in_palette(
...
def get_array_by_color(array_by_index: np.ndarray, palette: np.ndarray) -> np.ndarray:
    """Convert an rgb_array with indexed color and its palette to an rgb_array with full color.
...
def get_palette_by_cell(palette: np.ndarray) -> dict[tuple[int, int, int], np.ndarray]:
    ...
    For example, the key (1, 2, 3) would contain an rgb_array of colors whose red channel
    ...
    0 and 15, yielding up to 4,096 keys. Keys whose corresponding cell_to_check does not have
    any colors in the palette are excluded from the dict.

    The resulting output dict narrows down the search for the color in a palette most
    similar to a specific provided color.

    Arguments:
        palette: Complete palette
    Returns:
        dict whose keys are a tuple of cell_to_check coordinates in the red, green and blue
        dimensions, and whose values are an rgb_array of palette colors each chell
    """
    palette_by_cell = {}
...
def get_closest_color_in_palette(
