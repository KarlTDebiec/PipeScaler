...
class PotraceProcessor(Processor):
    ...
    def __init__(self, convert_executable: str = "convert",
                 potrace_executable: str = "potrace", blacklevel: float = 0.3,
                 alphamax: float = 1.34, opttolerance: float = 0.2,
                 **kwargs: Any) -> None:
    ...
    def process_file_in_pipeline(self, infile: str, outfile: str) -> None:
        self.process_file(infile, outfile, self.convert_executable,
                          self.potrace_executable, self.blacklevel,
                          self.alphamax, self.opttolerance,
    ...
    def construct_argparser(cls) -> ArgumentParser:
        ...
        parser.add_argument(
            "-c", "--convert_executable",
            default="convert",
            type=str,
            help="path to ImageMagick convert executable")
        parser.add_argument(
            "-p", "--potrace_executable",
            default="potrace",
            type=str,
            help="path to potrace executable")
        parser.add_argument(
            "-k", "--blacklevel",
            default=0.3,
            type=float,
    ...
    @classmethod
    def process_file(cls, infile: str, outfile: str, convert_executable: str,
                     potrace_executable: str, blacklevel: float,
                     alphamax: float, opttolerance: float, verbosity: int):

        # Convert to bmp; potrace does not accept png
        bmpfile = f"{splitext(infile)[0]}.bmp"
        command = f"{convert_executable} " \
                  f"{infile} " \
                  f"{bmpfile}"
        if verbosity >= 1:
            print(command)
