...
description: Audit and correct module hierarchy comments and exported symbol ordering in pipescaler __init__.py files using dependency-aware "bubble up" ordering, including cycle detection. Use when package structure changes or when hierarchy comments may be stale.
---
...
1. Identify `__init__.py` files:
   - `pipescaler/__init__.py` (top-level package hierarchy)
   - nested package `__init__.py` files
2. Run the helper script:
   - `python skills/audit-module-hierarchy/scripts/audit_module_hierarchy.py --target pipescaler`
3. Review output:
...
- For repository-wide architecture, analyze top-level packages under `pipescaler/`.
- For local package docs, analyze direct children of that package only.
